...
**Sentiment** is computed using the VADER (Valence Aware Dictionary and sEntiment Reasoner) lexicon, producing a compound polarity score ranging from −1 (most negative) to +1 (most positive). VADER is well-suited for song lyrics as it was designed for informal expressive text and is sensitive to capitalization, punctuation, and intensifiers.
...
- **Logistic Regression** serves as the interpretable baseline, trained in multinomial mode with L2 regularization (C = 1.0, lbfgs solver, up to 2,000 iterations).
...
Six configurations are compared on the validation set to select the final model. @tbl-comparison summarizes macro-averaged F1 for all configurations.

```{python}
#| echo: false
#| label: tbl-comparison
#| tbl-cap: "Macro-averaged F1 on the validation set for all model configurations. Chance performance for five balanced classes is 0.20."
...
All models substantially outperform the random-chance baseline of 0.20. XGBoost trained on the full 389-dimensional feature space achieves the highest validation F1, consistent with gradient-boosted trees' ability to perform implicit feature selection — making explicit PCA reduction unnecessary for this model class. Logistic regression performs competitively despite its simplicity, suggesting that a meaningful share of decade information is encoded in linearly separable directions within the embedding space. Based on these results, **XGBoost (Full)** is selected as the final model for test set evaluation and interpretation.
...
#| echo: false
#| label: fig-f1-bar
#| fig-cap: "Per-class F1 for XGBoost (Full) on the held-out test set. The dashed line marks random-chance performance (0.20)."
...
#| echo: false
#| label: fig-cm
#| fig-cap: "Confusion matrix for XGBoost (Full) on the held-out test set. Off-diagonal mass is concentrated between adjacent decades, consistent with gradual lyrical evolution rather than discrete decade-by-decade shifts."
...
Misclassifications are concentrated on the immediate off-diagonal: decades are confused with their temporal neighbours rather than with eras two or more decades away. The 1990s–2000s boundary is the most permeable, followed by the 2000s–2010s transition. The 1980s are most frequently confused with the 1990s, while the 2020s are occasionally predicted as 2010s. This pattern is consistent with gradual lyrical drift rather than sharp era boundaries.
...
#| echo: false
#| label: fig-shap
#| fig-cap: "Top 20 features by mean absolute SHAP value for XGBoost (Full). Engineered features are shown in orange; embedding dimensions in blue. Engineered features rank highly relative to individual embedding dimensions, validating the feature engineering effort."
import joblib, shap
MODELS_DIR = "models"
...
To test the secondary hypothesis, we compare Genius pageview counts between time travelers and correctly classified modern songs across the full dataset of 2010s/2020s tracks with available pageview data. @fig-nostalgia shows the distribution comparison, and a one-sided Mann-Whitney U test assesses whether time travelers attract systematically greater engagement.

```{python}
#| echo: false
#| label: fig-nostalgia
#| fig-cap: "Genius pageview distributions for modern songs (2010s/2020s) split by whether the model predicts them as vintage (1980s/1990s). Displayed on a log scale to accommodate the wide range of values."
fig = px.violin(
...
fig.add_annotation(
    ...
    text=f"Mann-Whitney U (one-sided, H₁: time travelers > classified): p = {pval:.3f}",
...
Time travelers have a median of **`{python} f"{tt_med:,.0f}"`** Genius pageviews compared to **`{python} f"{cor_med:,.0f}"`** for correctly classified modern songs. The Mann-Whitney U test yields p = `{python} f"{pval:.3f}"`. At α = 0.05, this result `{python} "supports" if pval < 0.05 else "does not support"` the secondary hypothesis that songs with vintage lyrical identity attract greater listener engagement.
...
On the second question, the pageview analysis `{python} "provides statistically significant evidence that time travelers attract greater engagement" if pval < 0.05 else "does not find statistically significant evidence that time travelers attract greater engagement"` at α = 0.05, `{python} "lending support to" if pval < 0.05 else "leaving open"` the nostalgia sells theory in the lyrical domain.
...
- **Adjacent decades are the hardest to distinguish.** The 1990s–2000s transition is the most confusing boundary. The 1980s and 2020s are the most distinctive, likely due to their particularly characteristic vocabularies — rock and dance-era terminology in the 1980s; contemporary slang and Latin influence in the 2020s.

- **XGBoost does not benefit from PCA.** The full-embedding XGBoost matches or outperforms the PCA-reduced variant, consistent with gradient-boosted trees' ability to perform implicit feature selection without explicit dimensionality reduction.
